...
m^{-(q-d)}p_m^{-(q-d)-(n-1)},\ \ \ \ &\mbox{for}~d+n\le q\le 1+(d-1)n,
  ...
  \textcolor{red}{\left(mp_m\right)^{-(q-d)}p_m^{-(n-1)}},\ \ \ \ \ &\mbox{for}~d+n\le q\le 1+(d-1)n. 
   \end{cases}
\end{align}
\textcolor{blue}{Hence, when $p_m\ll m^{-1}$, we have} 
$$
m^dp_m^{d+1}\gg m^q p_m^{\zeta(n,d,q)}, \quad 
 q=d+1,\ldots , 1+(d-1)n. 
$$
\textcolor{blue}{Moreover, when $m^{-1+1/d}\gg p_m\gtrsim m^{-1}$, we have $mp_m^2\ll 1$ as $d\ge2$, and also $mp_m\gtrsim1$. Hence,  if $m^{-1+1/d}\gg p_m\gtrsim m^{-1}$, 
\begin{align*}
	(mp_m)^{-(q-d)}p_m^{-(n-1)}&\gtrsim (mp_m)^{-(d-1)(n-1)}p_m^{-(n-1)}\\
					       &=[(mp_m)^{d-1}p_m]^{-(n-1)}\gg1
\end{align*}
for $d+n\le q \le 1+(d-1)n$.
In conlusion, from \eqref{cum-asy1}, we know that when $p_m\ll m^{-1+1/d}$, 
$$
m^dp_m^{d+1}\gg m^qp_m^{\zeta(n,d,q)},
$$
for $q=d+1,\dots,1+(d-1)n$. 
}
 And given that $\zeta (n,d,d) = d+1$,
 from \eqref{fjkldf} we obtain, for $m$ large enough, \textcolor{blue}{when $p_m\ll m^{-1+1/d}$}
\begin{align*}
