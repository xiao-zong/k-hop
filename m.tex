...
\title{\huge Phase transition for $k$-hop connectivity in one-dimensional binomial random\textcolor{red}{-connection models}} % -connection models}}
...
with a location-dependent probability. 
 ...
 Our focus is on multi-hop connecticity, specifically the number of paths of length $k$ (referred to as $k$-hop paths) connecting two fixed endpoints. Multi-hop connectivity plays a central role in applications including wireless networks, device-to-device and/or station-to-device communications in cellular networks, and spread of infection. 
%   \vspace{-11cm}
 \begin{figure}[H]
   \begin{center}
   \begin{tikzpicture}
...
%   \caption{Unit disk random-connection model.}
 ...
 \end{figure}
   ...
   \noindent
% \noindent
   \textcolor{red}{Specifically, we examine a system where nodes are distributed over the interval 
...
\begin{equation*}
    \kappa_n\big(\widebar{\sigma}_k (t)\big)\leq \begin{cases}
      ...
      \\
      \displaystyle
      \frac{C n!^{d+1}}{\big(m^d p_m^{d+1}\big)^{n/2-1}} & \text{when } \textcolor{blue}{p_m\ll m^{-1+1/d}},
    \end{cases}
\end{equation*}
...
\begin{enumerate}[i)] 
  ...
  see Corollary~\ref{graphcontain}-$i)$,
   and the normalized count satisfies the Kolmogorov bound 
    \begin{equation}
   \nonumber % \label{fjkf}
...
\item {\bf Sparse regime}, $p_m\ll m^{-1}$ .
...
\end{equation}
  ...
  and having precisely $\textcolor{red}{q}$ blocks, and 
\begin{equation}
...
\begin{align*}
  ...
  \atop
      ...
      \kappa(u_{\textcolor{red}{t}} (\mathbf{U}_{\alpha_1})
  ,
  \ldots
  ,
  u_{\textcolor{red}{t}} (\mathbf{U}_{\alpha_n})
  )
\\ 
   &\textcolor{red}{\le}
n^{n-1}
...
\item {\bf Dense regime}, $p_m\gg m^{-1+1/d}$.
 ...
 \begin{equation}
  ...
  \end{equation}
\item {\bf Sparse regime}, $\textcolor{blue}{p_m\ll m^{-1+1/d}}$.
   For $m$ large enough, we have 
    \begin{equation}
      \label{upp-2}
      |\kappa_n(\sigma_k (t))|\leq
...
\noindent
$ii)$ \textcolor{blue}{For} all $q=d+1,\ldots, 1+(d-1)n$, we have
\begin{align}
  \label{fjkldfaa1-2}
...
m^{-(q-d)}p_m^{-(q-d)-(n-1)},\ \ \ \ &\mbox{for}~d+n\le q\le 1+(d-1)n,
  ...
  \\
  \textcolor{red}{\left(mp_m\right)^{-(q-d)}p_m^{-(n-1)}},\ \ \ \ \ &\mbox{for}~d+n\le q\le 1+(d-1)n. 
   \end{cases}
\end{align}
\textcolor{blue}{Hence, when $p_m\ll m^{-1}$,
  for all
 $q=d+1,\ldots , 1+(d-1)n$, we have} 
$$
m^dp_m^{d+1}\gg m^q p_m^{\zeta(n,d,q)}. 
$$
\textcolor{blue}{On the other hand,
  when $m^{-1+1/d}\gg p_m\gtrsim m^{-1}$, we have $mp_m^2\ll 1$ since
...
$$
\textcolor{blue}{m^dp_m^{d+1}\gg m^qp_m^{\zeta(n,d,q)}.}
$$
 Hence, given that $\zeta (n,d,d) = d+1$,
 \textcolor{blue}{when $p_m\ll m^{-1+1/d}$} 
...
\item If $p_m\geq m^{-1+1/d}$, $m\geq 1$, then we have 
  \begin{equation}
    ...
    n^{n-1}n!^d m^{1+(d-1)n}p_m^{nd+1},
     \quad m \geq 1. 
  \end{equation}
\item If $p_m\leq \textcolor{blue}{m^{-1+1/d}}$, $m\geq 1$, then we have 
    \begin{equation}
    \nonumber %   \label{upp-2}
      |\kappa_n(\sigma_k (t))|\leq
      n^{n-1}n!^d 
...
\\
\nonumber
& = \sum_{\textcolor{red}{\rho\in {\rm NC} ([2]\times[d])}}
p_m^{|\rho|}
\left(
 ...
 satisfies 
  \begin{eqnarray*}
    \kappa_n\left(\widebar{\sigma}_k (t)\right)\leq \begin{cases}
      \displaystyle
      \frac{C n!^{d+1}}{(m p_m)^{n/2-1}},~~~~~~~&\mathrm{when}~~~p_m \gg m^{\textcolor{red}{-1+1/d}},
      \medskip
      \\
      \displaystyle
      \frac{C n!^{d+1}}{\big(m^d p_m^{d+1}\big)^{n/2-1}},~~~~&\mathrm{when}~~~p_m\ll m^{\textcolor{blue}{-1+1/d}},
    \end{cases}
\end{eqnarray*}
 ...
 satisfies  
  \begin{eqnarray*}
    \kappa_3\left(\widebar{\sigma}_k (t)\right)\leq \begin{cases}
      \displaystyle
      \frac{C}{(m p_m)^{1/2}}~~~~~~~&\mathrm{when}~~~\textcolor{red}{p_m} \gg m^{-1+1/d},
      \medskip
      \\
      \displaystyle
      \frac{C}{(m^d p_m^{d+1})^{1/2}}~~~~&\mathrm{when}~~~\textcolor{blue}{p_m\ll m^{-1+1/d}},
    \end{cases}
\end{eqnarray*}
...
\begin{corollary}
\label{thm4.2-1}
\textcolor{blue}{(Kolmogorov bound)}. 
...
\noindent
\textcolor{blue}{Let $d\geq 2$, $t\in [(k-1)r,kr]$. Then, for $m$ large enough, we have 
    \begin{equation*}
      \sup_{x\in\real}\left|
      \P_m \left(
...
\end{cases}
    ...
    $d \geq 2$. 
\end{corollary}
In particular, \textcolor{blue}{
\begin{enumerate}
	\item[i)]
if
 $p_m \sim m^{-\alpha}$ with $\alpha \in [0,1 - 1/d)$, we have 
    \begin{equation}
\nonumber 
      \sup_{x\in\real}\left|
      \P_m \left(
      \widebar{\sigma}_k (t) \leq x\right)-\Phi(x)\right|\leq \frac{C_d}{m^{(\textcolor{red}{1-\alpha} )/(2+4d)}},
    \end{equation}
    where $C_d>0$ depends only on
    $d \geq 2$;
\item[ii)]
	if $p_m \sim m^{-\alpha}$ with $\alpha \in [1 - 1/d,\frac{d}{d+1})$, we have 
    \begin{equation}
\nonumber 
      \sup_{x\in\real}\left|
      \P_m \left(
...
\lim_{m\to \infty}
 ...
 \begin{eqnarray}
   ...
   = 1.
 \end{eqnarray}
 On the other hand, if $p_m\ll \textcolor{blue}{m^{-d/(d+1)}}$, from
...
\newblock Asymptotic Normality of Subgraph Counts in Sparse Inhomogeneous Random Graphs.
\newblock {\em arXiv preprint arXiv:2512.12937.}
