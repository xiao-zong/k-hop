...
\title{\huge Normal approximation of $k$-hop counts in the binomial 1D unit disk model} % Main lemmas} 
...
%  Email: \href{mailto:xiaozong30@gmail.com}{xiaozong30@gmail.com}}
  \qquad
      Nicolas Privault\footnote{Division of Mathematical Sciences, School of Physical and Mathematical Sciences, Nanyang Technological University, 21 Nanyang Link, Singapore 637371, Email: \href{mailto:nprivault@ntu.edu.sg}{nprivault@ntu.edu.sg}
}
}
      %\author{ } 
...
\begin{abstract} 
  ...
  Normal approximation results with quantitative rate
...
% \noindent
 ...
 $\eta$ with $m_i \geq 0$ points on every
 cell $[(i-1)r, ir]$, $i=1,\ldots , k$, $k\geq 1$. 
 In addition, two nodes located $s,t\in [0,kr]$ are said to be
 connected - with probability $p\in [0,1]$ - if and only if $|t-s|\leq r$,
 and for $k \geq 1$, a $k$-hop is
 a path connecting two fixed points $x,y\in [0,kr]$
 via \underline{$d:=k-1$} nodes. 

 \medskip

% \noindent
 We are interested in the count $\sigma_k ( t )$ of 
 $k$-hops connecting two fixed endpoints
 located respectively at $0$ and $t$ for some $t \in [0,kr]$.
 In this context, 
the distribution of $k$-hop counts has been
expressed by a combinatorial approach in \cite{giles-privault}. 
 We will consider the statistics and asymptotic behavior of
 $k$-hop connectivity as the number of binomial
 points tends to infinity.
% When $k=1$ we have $\sigma_1(t) =1$, $t\in [0,r]$. 
 % where for later convenience of notation we model $\sigma_k$ as a function of the remaining distance $kr-\tau$ from $t$ to $kr$.
 For this, we will use the cumulant method, see 
 \cite{rudzkis},
 \cite{saulis},
 \cite{khorunzhiy},
 \cite{doring},
...
% \cite{jansen},
 ...
 in \cite{LiuPrivault}. 
% We will also use the Stein method, see 
% \cite{reitzner}, 
% \cite{eichelsbacher},
% \cite{lachieze-rey}, 
% \cite{doblerpeccati}, 
...
\medskip
 ...
 Denoting by 
 $\kappa_{k,n}^{( m )} (\underbrace{kr-t;\ldots ; kr-t}_{n~\mbox{\scriptsize times}})$
 the cumulant of order $n\geq 2$ of 
 $\sigma_k( t )$, for $t\in [rd, rk)$
 and $m_1=\cdots = m_d = m$, we obtain the bound   
$$ 
 \frac{ 
 \kappa_{k,n}^{( m )} (kr-t;\ldots ; kr-t)}{ \big(\kappa_{k,2}^{( m )}(kr-t;kr-t) \big)^{n/2}}
 \leq 
 (n!)^{d+1} O ( m^{1-n/2} )
, \qquad n \geq 2. 
$$
 Next, denoting by $\P_m$ the distribution of the 1D unit disk model 
 with same number $m$ of points per cell, by the cumulant method
 this implies the Komogorov distance bound \eqref{fjkf}
for the convergence of the normalized $k$-hop count 
$$
\widebar{\sigma}_k (t):= \frac{\sigma_k (t)- \E_m [\sigma_k(t)]}{\sqrt{\Var_m [ \sigma_k(t)]}}
$$
to the normal distribution ${\cal N}$ 
as $m$ tends to infinity.
 In Corollary~\ref{thm4.2-1}, 
 assume that $m^{-(d-1)/d}\ll p_m$, 
 we obtain the Kolmogorov bound 
    \begin{equation}
   \nonumber % \label{fjkf}
      \sup_{x\in\real}\left|
      \IP\left(
      \widebar{\sigma}_k (kr-\tau) \leq x\right)-\Phi(x)\right|\leq \frac{C_d}{(m p_m)^{1/(2+4d)}},
    \end{equation}
    for $m$ large enough, 
    ...
    If $p_m\ll m^{-1}$
    then  
    we have $\lim_{m\to\infty}\IP_m\left(\sigma_k(t)=0\right)=0$ Corollary~\ref{graphcontain} 
    by the second moment method.

    % In Proposition~\ref{fsklf34} we also obtain the Berry-Esseen bound
% $$\sup_{x\in \real} | \P_m ( \widebar{\sigma}_k (t) \leq x ) - \P({\cal N} \leq x ) | \leq \frac{C(k,r)}{\sqrt{m}} $$ using the Stein method, together with a bound of same order for the Wasserstein distance.

 \medskip
 In Theorem~\ref{poi-thm}, we provide a Poisson approximation of the $k$-hop count via Stein-Chen method. In particular, when $m^dp_m^{d+1}\to c\in(0,\infty)$, in Corollary~\ref{c2} 
 we show that the error bound for the Poisson approximation is of the order  
\begin{align}
  \dtv(W,\Pn(\lambda))&
  \asymp m^{-1/(d+1)}.
\nonumber % \label{po-error1}
\end{align}
as $m_1=m_2=\cdots=m_d=m$ tends to infinity.

\medskip 
The paper is organized as follows. Section~\ref{sec3} formulates the $k$-hop count as a generalized $U$-statistic. Section~\ref{sec-prel} introduces the necessary diagrammatic and set-partition preliminaries, while Section~\ref{sec4} presents moment expressions as summations over non-crossing partitions. Moment and cumulant bounds are derived in Section~\ref{sec6}, leading to a normal approximation for the normalized $k$-hop counts with Berry--Esseen rates via the Stein and cumulant methods in Section~\ref{sec8}. Finally, Section~\ref{sec-pol} establishes a Poisson approximation using the Stein--Chen method.
...
For each $\ell=1,\dots,d$, let $\{X^{(\ell)}_i\}_{i=1}^{m_\ell}$, $m_\ell\ge1$, be a sequence of i.i.d. random variables with a common distribution $\tilde{\mu}_\ell$, and be independent of all others.
Denote 
$$\mathcal{P}:=\cup_{\ell=1}^{d}\left\{X^{(\ell)}_{1},\dots,X^{(\ell)}_{m_\ell}\right\}\cup\{0,y\},$$ 
...
We focus on the count of $k$-hop, that is a path of order $k+1$, connecting $0$ to $kr-\tau$ in the random connection model $\mathbb{G}_p(\mathcal{P})$.
More formally, we can write it as 
\begin{equation}
	\sigma_k (kr-\tau):=\sum_{\beta\in[m_1]\times\cdots\times[m_{d}]}\bone(0\leftrightarrow X_{\beta(1)}^{(1)})\left(\prod_{\ell=1}^{k-2}\bone\left(X_{\beta(\ell)}^{(\ell)}\leftrightarrow X_{\beta(\ell+1)}^{(\ell+1)}\right)\right)\bone(X_{\beta(d)}^{(d)}\leftrightarrow y),\label{def-khop1}
\end{equation}
with $[n]:=\{1,2,\dots,n\}$.

 
 As noted in e.g. \cite{giles-privault}, when $\tau \in [0,r]$,
any node contributing to a $k$-hop path linking $0$ to $kr-\tau$
...
% on a finite interval, see \cite{drory1997}. 
% and \cite{penrosebk} for the more general setting of random geometric graphs.
...
\caption{Graph of seven $5$-hop paths linking $x=0$ to $y=4.5$ with $r=1$ and $d=4$.} 
\label{fklds} 
\end{figure}

% see Figure~\ref{fklds} for an example with $k=5$, $x=0$, $y=4.5$, and $r=1$.

\vskip-0.4cm

\noindent
 Conversely, any $k$-hop path linking $0$ to $kr-\tau$
 ...
 $$
 where $x_0=0$ and $x_k=kr-\tau$.  
% \subsubsection*{One-dimensional random geometric graphs} 
% \label{s2}
Therefore, it can be mapped to a
 sequence $(y_1,\ldots , y_d) \in [0,\tau]^d$
 by the relation 
 $$
 y_i:=x_i - (ir-\tau), \qquad i =1,\ldots , d, 
 $$ 
 with $ y_1 \ge \cdots \ge y_d$. 

 Hence, for $\tau \in [0,r]$, the count $\sigma_k (kr-\tau)$ in \eqref{def-khop1} of $k$-hop 
 connecting $0$ to $kr-\tau$ can be rewritten as a generalized $U$-statistics of order $d$
\begin{equation}
\label{ustat} 
 \sigma_k (kr-\tau) = \sum_{\beta \in [m_1] \times \cdots \times [m_d]} 
 Y^{(0)}_{\beta (1)}Y^{(1)}_{\beta (1),\beta (2)}\cdots Y^{(d-1)}_{\beta (d-1),\beta (d)}
 Y^{(d)}_{\beta (d)}
 {\bf1}_{\big\{\textcolor{red}{U^{(d)}_{\beta (d)} \leq \cdots \leq U^{(1)}_{\beta (1)} \leq \tau }\big\}},
\end{equation}
where
 $\left\{
 Y^{(0)}_{\beta (1)}
 ...
 Bernoulli random variables with same parameter $p\in [0,1]$ with 
 $$
 U^{(\ell)}_{i}:=X^{(\ell)}_{i}-(\ell r-\tau),
 $$
for $i=1,\dots,m_\ell$ and $\ell=1,\dots,d$. 

For ease of notation, from now on, we write the distribution of $U^{(\ell)}_i$ as 
$\mu_{\ell}:=\tilde{\mu}_\ell*\delta_{-(\ell r-\tau)},
$ for $i=1,\dots,m_\ell$ and $\ell=1,\dots,d$.

 \medskip
  
Note that for $k=2$, $p=1$ and $\tau = r$, we have $\sigma_2 (r) =m_1$ almost surely.
...
In what follows, we let $[n]:=\{1,2,\dots,n\}$ for $n\geq 1$,
 and let $\Pi(b)$ denote the collection of set partitions of
any finite set $b$.
...
denote the single-block coarsest partition of $b$,
 ...
 Given $k\geq 2$ and $j\geq 1$,
 we let $\pi:=\{\pi_1,\dots,\pi_j\}$ % \in\Pi([j]\times[d])$
 denote the partition
 of $[j]\times[d]$ defined as 
 $$\pi_i:=\{(i,\ell):1\leq \ell\leq d\},
 \quad i =1,\dots , j.
 $$
 For $j,k \geq 1$ we also let $\pi_\eta := (\pi_i)_{i\in\eta}\in\Pi(\eta \times [d])$ denote the partition made of $|\eta|$ blocks of size $d$.
 A partition $\rho\in\Pi([j]\times[d])$ is said to be {\it connected} if $\rho\vee\pi=\widehat{1}$, see Figure~\ref{fig:diagram0-1}.  
...
\begin{definition}
 ...
 $$
 where $\rho_i$ is a partition of $\{(i,j)\}_{j=1,\ldots , n}$, 
% As above, we let $\lambda_l(s):=\lambda ( s)$, $l=1,\ldots , d$. 
 see Figure~\ref{fig:diagram0-1-2}.  
% \vspace{-0.3cm}
\begin{figure}[H]
\captionsetup[subfigure]{font=footnotesize}
\centering
...
\end{pgfonlayer}
\end{tikzpicture}}
\caption{Examples of partition diagrams with $n=5$ and $r=4$.}
...
\end{pgfonlayer}
\end{tikzpicture}}
  \caption{Example for the mapping $\sqcap$ with $n=5$ and $k=4$.}
  \label{fig:diagram1}
  \end{figure}
...
\noindent 
 We also have the following lemma, which is analog to
...
\end{Proof} 
\noindent
   Finally, we will use the following standard notation for the asymptotic behavior of the relative order of magnitude of two functions $f(x)$ and $g(x)>0$ as $x$ tends to infinity.
Given two functions $f(x)$ and $g(x)>0$, 
 we write 
\begin{itemize}
...
\begin{prop}
\label{p1}
 Let $n \geq 1$. For any $\tau_1,\ldots , \tau_n \in [0,r]$,
 we have 
\begin{align} 
\label{djkls12}
& \E  [ \sigma_k(kr-\tau_1) \cdots  \sigma_k(kr-\tau_n)  ] 
\\
\nonumber
& = \sum_{
% \substack{\rho\in {\rm NC} ([n]\times[d])\\\rho\wedge\pi=\widehat{0}}
...
where
$$
c(\rho ) : = |\rho_1|+|\rho_d|+\sum_{i=1}^{d-1} \min ( |\rho_i|,|\rho_{i+1}|), 
 $$
  and 
 $u_\tau : [0,r]^d \mapsto \{0,1\}$ is the indicator function
 of $d$ variables given by 
 \begin{equation}
   \label{jklf12} 
 u_\tau (x_1 , \ldots , x_d)
 =
 {\bf1}_{\{x_1 \leq \cdots \leq x_d \leq \tau \}}
% \prod_{p=1}^d {\bf 1}_{\{m_1+\cdots +m_{p-1} < i_p \leq m_1+\cdots + m_p\}}
 ,
 \quad
 x_1,\ldots , x_d \in [0,r], 
\end{equation} 
 and 
 $\bar{\rho}_i$ denotes the index $j\in \{1,\ldots , d \}$ of the unique block
 $\eta_j = ((i,j))_{i=1,\ldots , n}$ containing $\rho_i$, $i=1,\ldots , |\rho|$. 
% where $\zeta^j_i$ denotes the block of $\pi^j$ that contains the index $i\in \{1,\ldots , n\}$,  and $dz^j_{\pi^j} := (dz^j_i )_{ i \in \pi^j}$, $j=1,\ldots , k-1$. 
\end{prop}
\begin{Proof}
...
\\
 ...
 u_{\tau_i} \big(U^{(1)}_{\alpha_i(1)} , \ldots , U^{(d)}_{\alpha_i(d)}\big) \Big) 
 \right]
 \\
 & = 
...
1\leq i \leq n}
 ...
 \\
 & = 
     \sum_{\substack{\rho\in {\rm NC}([n]\times [d])\\\rho\wedge\pi=\widehat{0}}}
        \sum_{\alpha_i\in [m_1]\times \cdots \times [m_d]
  \atop
1\leq i \leq n}
        \hskip-0.8cm
 ...
 \right]
    ...
    &       = \sum_{\substack{\rho\in{\rm NC}([n]\times [d])\\\rho\wedge\pi=\widehat{0}}}
 p^{e(\rho_G)}
    \sum_{\alpha_1\in [n]^k,\ldots,\alpha_n\in[n]^k
    \atop \sqcap (\alpha ) = \rho }
    \int_{[0,r]^{|\rho|}} \left(\bigotimes_{i=1}^n u_{\tau_i} \right)_{\! \! \! \rho}
      ( x_1, \ldots , x_{|\rho|} ) 
 \mu_{\bar{\rho}_1} ( d x_1 ) 
 \cdots
 \mu_{\bar{\rho}_{|\rho|}} ( d x_{|\rho|} ) 
...
C_{m_1,\ldots , m_d} (\rho)
 p^{e(\rho_G)}
\int_{
   [0,r]^{|\rho|}}\left(\bigotimes_{i=1}^n u_{\tau_i} \right)_{\! \! \! \rho}
  ( x_1, \ldots , x_{|\rho|} ) 
 \mu_{\bar{\rho}_1} ( d x_1 ) 
 \cdots
 \mu_{\bar{\rho}_{|\rho|}} ( d x_{|\rho|} ) 
...
& \E  [ \sigma_2(2r-\tau_1) \cdots  \sigma_2(2r-\tau_n)  ] 
\\
\nonumber
& \quad = \sum_{\rho\in \Pi ([n] )} 
...
\noindent
 ...
 and 
 the first moment of the count $\sigma_k$ of $k$-hop paths is
 given by
\begin{align} 
\nonumber
 \E [ \sigma_k (kr-\tau) ]
 ...
 u_\tau \big(U^{(1)}_{\beta (1)} , \ldots , U^{(d)}_{\beta (d)}\big) 
 \right]
\\
\nonumber
 & = 
...
\nonumber
 ...
 p^{d+1}
 m_1\cdots m_d
 \int_0^\tau \int_0^{x_d} \cdots \int_0^{x_2} \mu_1(dx_1)\cdots \mu_d(dx_d). 
\end{align}
...
& = 
 \frac{p^{d+1}}{r^d}
 m_1\cdots m_d 
 \int_0^\tau \int_0^{x_d} \cdots \int_0^{x_2} dx_1\cdots dx_d
% {\bf1}_{\{i_1<\cdots < i_d\}} \prod_{p=1}^d {\bf 1}_{\{m_1+\cdots +m_{p-1} < i_p \leq m_1+\cdots + m_p\}}
\\
\label{fjkdls-0}
& = 
p^{d+1} m_1\cdots m_d\frac{(\tau / r)^d}{d!}. 
\end{align} 
...
$$ 
 and 
$\sigma_k(kr-\tau)$ is a Bernoulli random variable with parameter
$$
 \P\big(U_1^{(1)} \leq \cdots \leq U^{(d)}_1 \leq \tau \big) =
 \frac{(p \tau )^d}{r^dd!}. 
$$ 
...
\begin{corollary} 
% \label{p1}
 For any $\tau_1,\tau_2 \in [0,r]$,
 we have 
\begin{align} 
\nonumber % \label{djkls12}
& \E  [ \sigma_k(kr-\tau_1) \sigma_k(kr-\tau_2)  ] 
\\
\nonumber
& = \sum_{
...
c(\rho ) : = |\rho_1|+|\rho_d|+\sum_{i=1}^{d-1} \min ( |\rho_i|,|\rho_{i+1}|), 
 ...
 vertical block
 $\eta_j = ((1,j),(2,j))$ containing $\rho_i$, $i=1,\ldots , |\rho|$. 
\end{corollary} 
\noindent
 In particular, in the case of $2$-hops with $d=1$, we have 
\begin{align} 
...
\noindent 
 ...
 second moment of the count $\sigma_k(kr-\tau)$ of $k$-hop paths.  
% Alternatively, this result can be proved as a consequence of Corollary~\ref{c1} and the It\^o isometry for multiple Poisson stochastic integrals. 
 Higher cumulants and moments of $\sigma_k(kr-\tau)$ may also 
 be computed by this method, 
 using % Corollary~\ref{c1} above and
 Corollary~7.4.1 of
 \cite{peccatitaqqu}. 
\begin{prop} 
\label{djklfs2}
Assume that $p=1$ and $\mu_i$ is the uniform measure on $[0,r]$ for all
$i=1,\ldots , d$.
 Then, for $k\geq 3$,
 the second moment of the count $\sigma_k(kr-\tau)$ of $k$-hop paths is given by
\begin{align} 
\nonumber % \label{fjkdls}
  & \E  [ \sigma_k(kr-\tau)^2 ]
  \\
...
(2(i_{j+1}-i_j-1))!}
     ...
     ,
 \end{align*} 
where we take $z_0=0$ and $z_k=\tau$,
 $$
...
\end{Proof}
 ...
 variance of the count $\sigma_k$ of $k$-hop paths is given by 
\begin{align} 
...
\label{djklfs2-2}
Assume that $p=1$ and $\mu_i$ is the uniform measure on $[0,r]$ for all
$i=1,\ldots , d$, and $m_1 = \cdots = m_d = m$. 
% $\lambda_i=\lambda$, and let
% $a_{k,i} = a_k$, $i=1,\ldots , k-1$. 
 Then, for $\tau \in [0,r]$, the variance 
...
& = 4^{d-l} \frac{\Gamma ( d-l + (l+1)/2)}{(d-l)!\Gamma ((l+1)/2)}, 
\end{align} 
 see Corollary~4.3 in \cite{privaultkhops}. 
\end{Proof}
...
\prod_{i=1}^d \lambda_i^{|\rho_i|}
 \int_{
  ...
  u_{\tau_i} \right)_{\! \! \! \rho}    ( x_1, \ldots , x_{|\rho|} ) 
 \mu_{\bar{\rho}_1} ( d x_1 ) 
 \cdots
 \mu_{\bar{\rho}_{|\rho|}} ( d x_{|\rho|} ) 
% (\mathbf{x})\mu^{\otimes |\rho|} ( \mathrm{d}\mathbf{x} )
\end{align} 
 ...
 Similarly, multiplying \eqref{fjkdls}
by $\displaystyle \prod_{i=1}^d e^{-\lambda_i}\frac{\lambda_i^{m_i}}{m_i!}$
and summing over $m_1,\ldots , m_d = 0$ to infinity, 
the expression
\eqref{fjkdls} of $\E  [ \sigma_k^2 ]$ 
...
% \scriptsize
 ...
 $$
 \kappa_{k,n}^{( m )} ( \widebar{\tau}_{\pi_1} ; \ldots ; \widebar{\tau}_{\pi_l} ) : = 
% with $|\pi| := ( |\pi_1|,\ldots , |\pi_l| )$ 
 \kappa_n \big( ( \sigma_k(kr - \tau_1) )^{|\pi_1|}, \ldots , ( \sigma_k(kr - \tau_l) )^{|\pi_l|}\big)
 $$ 
 the joint cumulant
 of $\big( ( \sigma_k(kr - \tau_1) )^{|\pi_1|}, \ldots , ( \sigma_k(kr - \tau_l) )^{|\pi_l|}\big)$, and we let 
 $\kappa_n( \sigma_k(kr - \tau ) )$ 
...
%  Let $f:[0,r]^d \to\real$ be a measurable function, with $d\geq 2$.
  For $m\geq 1$, 
  the $n$-$th$ cumulant of the $k$-hop count $\sigma_k(kr - \tau )$ 
  satisfies the bound   
  \begin{equation}
    \label{fjkldf}
    | \kappa_n( \sigma_k(kr - \tau ) )|\leq
    n^{n-1}
    % n!^d  
  % |{\rm CNC}([n]\times [d])|
    \sum_{q=d}^{1+(d-1)n}
    |\mathcal{C}(n,d,q)| m^q p^{\zeta (n,d,q)} 
, \qquad n\geq 1, 
\end{equation}
...
\begin{Proof}
  ...
  $\kappa(
  u_{\tau_1} (\mathbf{U}_{\alpha_1})
  ,
  \ldots
  ,
  u_{\tau_n} (\mathbf{U}_{\alpha_n})
  )$
  vanishes.
  Hence, we have 
  \begin{align}
    \nonumber
    \kappa_n(
    \sigma_k(kr-\tau_1), \ldots , \sigma_k(kr-\tau_n) 
    )&= \sum_{\alpha_i\in [m_1]\times \cdots \times [m_d]
  \atop
 1 \leq i \leq n
    }
  ...
  \atop
      ...
      \kappa(        u_{\tau_1} (\mathbf{U}_{\alpha_1})
  ,
  \ldots
  ,
  u_{\tau_n} (\mathbf{U}_{\alpha_n})
...
\prod_{i=1}^l\left|
  \E\left[\prod_{\ell\in b_i}
    u_{\tau_\ell} (\mathbf{U}_{\alpha_\ell})
  \right]
\right|
\\
\nonumber  &\leq
...
\begin{enumerate}[a)]
\item If
 $p_m\gg m^{-(d-1)/d}$, then for $m\geq 1$ large enough 
 we have 
\begin{equation}\label{upp-1}
    |\kappa_n(\sigma_k (kr-\tau))|\leq
   n^{n-1}n!^d m^{1+(d-1)n}p_m^{nd+1}. 
  \end{equation}
\item If $p_m\ll m^{-1}$, then for $m\geq 1$ large enough we have 
    \begin{equation}
      \label{upp-2}
      |\kappa_n(\sigma_k (kr-\tau))|\leq
      n^{n-1}n!^d 
...
\begin{align}
  ...
  \nonumber
        \\
							    &=
	\begin{cases}m^{1+(d-1)n-q}p_m^{(n+1)d-2q},\ \ \ &\mbox{for}~d\le q\le d+n-1;\\[0.5ex]
...
\end{align*}
 ...
 from \eqref{fjkldf} we obtain,
 \textcolor{purple}{for $m\geq 1$ large enough}, 
\begin{align*}
  \nonumber
  \vert\kappa_n(\sigma_k (kr-\tau))\vert
  &\leq n^{n-1}
...
$$
 ...
 \textcolor{purple}{for $m\geq 1$ large enough}, 
\begin{align*}
  \vert\kappa_n(\sigma_k (kr-\tau))\vert &\leq n^{n-1}
  \sum_{q=d}^{1+(d-1)n} |\mathcal{C}(n,d,q)|m^q p_m^{\zeta (n,d,q)}\\
...
\end{align*}
\end{Proof}
\textcolor{purple}{By inspection of the proof of
...
\begin{enumerate}[a)]
\item If
 $p_m\geq m^{-(d-1)/d}$, $m\geq 1$, then we have 
  \begin{equation}
    \nonumber % \label{upp-1}
    |\kappa_n(\sigma_k (kr-\tau))|\leq
    n^{n-1}n!^d m^{1+(d-1)n}p_m^{nd+1},
...
% Suppose that $\limsup_{m\to \infty} p_m < 1$. 
 Let $d\geq 2$. 
\begin{enumerate}[a)]
\item If $m^{-1} \ll p_m$, we have % the equivalence  
  \begin{equation}
    \label{low-1}
    \Var_m [\sigma_k (kr-\tau)]
% \geq C \frac{m!}{(m+1-2d)!}p_m^{2d}
    \sim 
    2 p_m^{2d+1} \frac{(2m \tau /r)^{2d-1}}{(2d-1)!}
. 
  \end{equation}
...
, 
\end{align} 
where $u_\tau ( x_1, \ldots , x_k )$ is
defined in \eqref{jklf12} and 
 $\bar{\rho}_i$ denotes the index $j\in \{1,\ldots , d \}$ of the unique block
 $\eta_j = ((i,j))_{i=1, 2}$ containing $\rho_i$, $i=1,\ldots , |\rho|$. 

 If $m^{-1} \ll p_m$, the dominant term
 in $\E  [ ( \sigma_k(kr-\tau ) )^2 ]$ 
...
$$ 
 ( \E [ \sigma_k (kr-\tau) ] )^2 = 
p_m^{2d+2} m^{2d} \frac{(\tau / r)^{2d}}{(d!)^2} 
$$
...
$$ 
 hence the dominant term in $\Var_m [ \sigma_k (kr-\tau) ]$ is
...
p_m^{2d+2} m^{2d} \frac{(\tau / r)^{2d}}{(d!)^2}, 
$$ 
 is dominated by the $2d$-block partition 
...
\begin{corollary}
  \label{cjkfl} 
% Suppose that $\limsup_{m\to \infty} p_m < 1$.
  For $n\geq 1$ large enough, % $n\geq 4(v(G)-1)$,
  the $j$-th cumulant of the normalized subgraph count
$$
  \widebar{\sigma}_k (kr-\tau):= \frac{\sigma_k (kr-\tau)-\E [ \sigma_k (kr-\tau)]}{\sqrt{\Var_m [ \sigma_k (kr-\tau) ]}}
  $$ 
 satisfies 
  \begin{eqnarray*}
    \kappa_n\left(\widebar{\sigma}_k (kr-\tau)\right)\leq \begin{cases}
      \displaystyle
      \frac{n!^{d+1}}{(C m p_m)^{n/2-1}}~~~~~~~{when}~~~m^{-(d-1)/d}\ll p_m,
      \medskip
      \\
      \displaystyle
...
% \begin{equation} j^{j-1}<\frac{(j-1)!}{\sqrt{2\pi j}}e^j. \end{equation}
  When $m^{-(d-1)/d}\ll p_m$, from the inequalities  
  \eqref{low-1} and \eqref{upp-1}, we have 
  \begin{align*}
    \nonumber
    \kappa_n\left(\widebar{\sigma}_k (kr-\tau)\right)&= \frac{\kappa_n(\sigma_k (kr-\tau))}{\Var_m [ \sigma_k (kr-\tau) ]^{n/2}}    \\
    \nonumber
    &\leq n!^d n^{n-1}m^{1+(d-1)n}p_m^{nd+1}
      \left(  
\frac{p_m^{2d+1} m^{2d-1} (2 \tau /r)^{2d-1}}{2(2d-1)!}
      \right)^{-n/2}
    \\
    \nonumber
    &\leq n!^{d+1} e^n m^{1-n/2}p_m^{nd+1}
    \left(  
 \frac{p_m^{2d+1} (2 \tau /r)^{2d-1}}{2(2d-1)!}
      \right)^{-n/2}
    \\
    \nonumber
    & = n!^{d+1} e^n m^{1-n/2} p_m^{1-n/2}
    \left(  
\frac{(2 \tau /r)^{2d-1}}{2(2d-1)!}
      \right)^{-n/2}. 
\end{align*}
...
$$
 ...
 and
 $$
 d_K(X,Y) : = \sup_{x\in \real} | \P ( X \leq x ) - \P( Y \leq x ) |.
 $$ 
...
$$
 ...
 of $\sigma_k (t )$ satisfies  
  \begin{eqnarray*}
    \kappa_3\left(\widebar{\sigma}_k (kr-\tau)\right)\leq \begin{cases}
      \displaystyle
      (C m p_m)^{-1/2}~~~~~~~{when}~~~m^{-(d-1)/d}\ll p_m,
      \medskip
      \\
      \displaystyle
...
\end{eqnarray*}
 ...
 tends to infinity, 
 provided that $m^{-1}\ll p_m$ as $m$ tends to
 infinity.

 \medskip
 
 From Corollary~\ref{cjkfl}, we check that if
 $m^{-1}\ll p_m$, the cumulants of the normalized
 $U$-statistics 
 $\widebar{\sigma}_k (kr-\tau)$ satisfy the Statulevi\v{c}ius growth condition 
...
\nonumber % \label{fjkf}
      \sup_{x\in\real}\left|
      \IP\left(
      \widebar{\sigma}_k (kr-\tau) \leq x\right)-\Phi(x)\right|\leq \frac{C_d}{m^{(1+\alpha )/(2+4d)}},
    \end{equation}
    where $C_d>0$ depends only on
    $d \geq 2$.
...
By Corollary~\ref{cjkfl} and 
Proposition~\ref{l1}-$ii)$,
...
$ 
  a_m \ll m^{1/(2+4 d)}$. 
   % as $n\to\infty$.
  Then, $\big(a_m^{-1} \widebar{\sigma}_k (kr-\tau) \big)_{m \geq 1}$ satisfies a moderate deviation principle with speed $a_m^2$ and rate function $x^2/2$. 
\end{corollary} 
 \noindent 
     Moreover, \eqref{fjkf} can be improved as a Berry-Esseen bound
     in the following proposition.
\begin{prop}
% \label{fsklf34}
  Assuming that $p_m = 1$, $m\geq 1$.
  Then, we have
  $$
  d_K(
  \widebar{\sigma}_k (kr-\tau)
  , {\cal N}) \leq \frac{C_k}{\sqrt{m}}, \qquad
  m \geq 1,
  $$
  where $d_K(\widebar{\sigma}_k (kr-\tau), {\cal N})$ denotes the Kolmogorov % Wasserstein
  distance between 
  $\widebar{\sigma}_k (kr-\tau)$ and the standard normal distribution ${\cal N}$, 
  and $C_k>0$ is a constant.
\end{prop}
\begin{Proof}
  We % note that $\widebar{\sigma}_k(n)$ admits a Hoeffding decomposition and
  apply \cite{zhangzs} for the Kolmogorov distance. 
\end{Proof}
The following thresholding result for $k$-hop containment
is a another direct consequence of Theorem~\ref{th7.5-1}. 
  \begin{corollary}
    \label{graphcontain}
    We have 
    \begin{enumerate}[a)]
      \item $\lim_{m\to\infty}\IP\left(N_G=0\right)=1$ if $p_m\gg m^{-(d-1)/d}$, 
      \item $\lim_{m\to\infty}\IP\left(N_G=0\right)=0$ if $p_m\ll m^{-1}$. 
    \end{enumerate}
  \end{corollary}
  \begin{Proof}
 When $p_m\gg m^{-(d-1)/d}$,
...
\nonumber
      \lim_{m\to \infty}
   \frac{\kappa_2(N_G)}{\kappa_1(N_G)^2} \lesssim 
\lim_{m\to \infty}
 ...
 \begin{eqnarray}
   \lim_{m\to \infty}
   \frac{(\E[N_G])^2}{\E[N_G^2]}
   =
   \lim_{m\to \infty} \frac{\kappa_1(N_G)^2}{\kappa_2(N_G)+\kappa_1(N_G)^2}
   = 1.
 \end{eqnarray}
...
\begin{equation}
          ...
          Y^{(0)}_{\beta (1)}Y^{(1)}_{\beta (1),\beta (2)}\cdots Y^{(d-1)}_{\beta (d-1),\beta (d)}
 Y^{(d)}_{\beta (d)}
 {\bf1}_{\big\{U^{(1)}_{\beta (1)} \leq \cdots \leq U^{(d)}_{\beta (d)} \leq \tau \big\}},
        \quad
...
\begin{equation*}
	...
	\begin{cases}1,\ \ \ &\mbox{if}\ \ d\in A;
          ...
          \\
	  Y_{\beta (d)}^{(d)}
          \bone_{\big\{ U_{\beta( d )}^{( d )} \leq \tau  \big\}}
          ,\ \ \ \ &\mbox{if}\ \ d\notin A.
	\end{cases}
\end{equation*}
\end{enumerate}
...
\begin{align}\label{count-parti}
	\nu(i)&= \begin{cases}(m_1-1)\times(m_2-1)\times\cdots\times(m_d-1),\ \ \ &\mbox{\rm when}~~i=0;\\
...
\begin{equation*}
  \dtv( X,Y):=\sup_{A\subseteq\Z}|\IP(X\in A)-\IP(Y\in A)|,
\end{equation*}
 with $\Z:=\{0,\pm1,\pm2,\dots\}$ standing for the set of signed integers.
% \section{Poisson approximation}
\begin{theorem}\label{poi-thm}
  Let $W=\sum_{\alpha\in\ci}I_\alpha$ be the $k$-hop count defined in
  \eqref{ustat}, with $I_\alpha$ given in
  \eqref{ibeta} and $\lambda=\E [ W ]>0$.
  Then, we have
\begin{align}
  \dtv(W,\Pn(\lambda))&\le
  \tilde{p}
...
\begin{align}
	\dtv( W ,\Pn(\lambda))&= \sup_{A\subseteq \Z}|\IP(W\in A)-\Pn(\lambda)\{A\}|\nonumber\\
...
\begin{align}
	\left|\E[\lambda f_A(W+1)-Wf_A(W)]\right|&\le\sum_{i=1}^{d-1}\nu(i)p_m^{d+1-i}+\tilde{p}
\end{align}
Combining with \eqref{steinest01}, the proof is complete. 
\end{Proof}
 We note that in order to ensure Poisson approximation, there is no need to force $\E [W]$ converges to a constant $\mu$. 
...
\cite{leonov},
\cite{malyshev1980},
and Proposition~3.2.1
 in \cite{peccatitaqqu}. 
...
\item (Kolmogorov bound,
 \cite[Corollary~2.1]{saulis} and \cite[Theorem~2.4]{doering})
  One has
\begin{equation*}
\sup_{x\in\real }|\IP(X_n\leq x)-\Phi(x)|\leq \frac{C(\gamma )}{(\Delta_n)^{1/(1+2\gamma)}},
\end{equation*}
...
\begin{equation*}
\IP(|X_n|\geq x)\le2\exp\left(-\frac14\min\left( \frac{x^2}{2^{1+\gamma}},(x\Delta_n)^{1/(1+\gamma)}\right) \right).
\end{equation*} 
...
\begin{eqnarray*} \frac{\IP(X_n\geq x)}{1-\Phi(x)}&=& \left(1+O\left(\frac{x+1}{(\Delta_n)^{1/(1+2\gamma)}}\right)\right) \exp \big( \widetilde{L}(x) \big), \\ \frac{\IP(X_n\leq -x)}{\Phi(-x)}&=& \left(1+O\left(\frac{x+1}{(\Delta_n)^{1/(1+2\gamma)}}\right)\right) \exp \big( \widetilde{L}(-x) \big), \end{eqnarray*} where $\widetilde{L}(x)$ is related to the Cram\'er-Petrov series. 
\end{enumerate}
\end{prop}
...
the normalized $k$-hop count
$$
\widebar{\sigma}_k := \frac{\sigma_k - \E[\sigma_k]}{\sqrt{\Var [ \sigma_k]}}
$$
...
\begin{align} 
\nonumber % \label{fjkdls}
  & \frac{(\E  [ \sigma_k(kr-\tau) ])^2}{\E  [ \sigma_k(kr-\tau)^2 ]} 
  = 
  \left(
...
\end{equation} 
 ...
 \sum_{l=1}^n
  ...
  \sum_{\pi_1\cup \cdots \cup \pi_l = \{1,\ldots , n\}}
  \prod_{j=1}^l 
  \E \Bigg[ \prod_{i\in \pi_j} Z_i^{l_i} \Bigg], 
$$
...
$$
  m_{k,2} (u_1,u_2) 
  =  \E [ \sigma_k(kr- u_1) \sigma_k(kr-u_2) ],
...
$$
  ...
  :=
  \E \big[ Z^{(k)}_{s_1} \cdots Z^{(k)}_{s_n} \big] 
$$
...
\begin{align*} 
\nonumber 
& \E  [ \sigma_k(kr-t_1) \sigma_k(kr-t_2)  ] 
\\
 & =  
...
\begin{align*} 
\nonumber 
& \E  [ \sigma_k(kr-t_1) \cdots  \sigma_k(kr-t_n)  ] 
\\
 & =  
...
\nonumber 
& m_{k+1,n}(t_1,\ldots , t_n)
 = \E  [ \sigma_k((k+1)r-t_1) \cdots  \sigma_k((k+1)r-t_n)  ] 
\\
& = 
% \hskip-0.3cm
 % \sum_{k=1}^{nr}  
...
\end{prop} 
 ...
 , t_n ) \big).
 $$
% Let $$ m_k(t_1,\ldots , t_n) = \E [ \sigma_k (t_1) \cdots \sigma_k ( t_n) ] $$  denote the joint moment of order $n$ of the $k$-hop count. 
...
\frac{c_{k,n} (t,\ldots , t)}{ (c_{k,2}(t,\ldots , t) )^{n/2}}
 & = & 
\frac{c_{k,n}(t,\ldots , t)}{( \Var [ \sigma_k(t)] )^{n/2}} 
\\
 & \simeq & 
...
&
 ...
 - 3 \E \big[ \big( Z^{(k+1)}_t \big)^2 \big]
 \E \big[ Z^{(k+1)}_t \big]
 + 2 \big(  \E \big[ Z^{(k+1)}_t \big] \big)^2 
\\
 & = 
  \int_0^t
  \E \big[ \big( Z^{(k)}_{s_1} \big)^3 \big] 
  ds_1
  +
  3 \int_0^t
  \int_0^t
  \E \big[ \big( Z^{(k)}_{s_1} \big)^2 Z^{(k)}_{s_3} \big]
  ds_1 ds_3 
 - 3   \int_0^t
\E \big[ \big( Z^{(k)}_{s_1} \big)^2 \big]
ds_1 
\int_0^t
\E \big[ Z^{(k)}_{s_1} \big]
ds_1
...
\\
  ...
  \int_0^t
  \int_0^t
  \E \big[ Z^{(k)}_{s_1} Z^{(k)}_{s_2} Z^{(k)}_{s_3} \big]
  ds_1ds_2ds_3
   + 2 \left( \int_0^t
\E \big[ Z^{(k)}_{s_1} \big]
ds_1 \right)^3. 
...
$$
 hence we have
\begin{eqnarray*} 
 \E [ \sigma_2(2r-t_1)\cdots \sigma_2(2r-t_n) ]
 & : = & 
  \E \big[ N^{(1)}_{t_1} \cdots N^{(1)}_{t_n} \big] 
  \\
  & = & 
...
In the case of three-hops, we have
 $\E  [ \sigma_3 (3r-t) ] = t^2 \lambda_1 \lambda_2 / 2$ 
 and 
 $$
 \Var [ \sigma_3 (3r-t)]
 = 
