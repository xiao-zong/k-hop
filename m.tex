...
\title{\huge Phase transition for $k$-hop connectivity in one-dimensional binomial random-connection models} % -connection models}}
...
with a location-dependent probability. 
 The binomial RCM, in particular, may be viewed as a spatial generalization of the Erd{\H o}s-R\'enyi model, 
 and has been extensively studied under various framework, including inhomogeneous random graphs,
 c.f. \cite{DevroyeFraiman14,penrose18,hladky21,bhattacharya25}, and graphon-based random graphs c.f. \cite{coulson16,zhangzs,bhattacharya23}.

 \medskip

 In this paper, we study a one-dimensional 
 unit disk binomial RCM with connection radius $r>0$
 on a finite interval, as illustrated Figure~\ref{fjkld23},
 see also \cite{drory1997,wilsher,han2007}
 for the case of one-dimensional random geometric graphs (RGGs),
 for which the connection probability is either $0$ or $1$, 
 and \cite{liebreview}
 for the corresponding one-dimensional statistical physics.
 One-dimensional RCMs and RGGs have been used to model
 Vehicular Ad-Hoc Networks (VANETs)
 \cite{ng2011,zhang2012,zzhang2014}
 or Mobile Ad-Hoc Networks (MANETs) \cite{foh2005},
 see \cite{shang2009} for access point connectivity.
 
\medskip
 
 Our focus is on multi-hop connectivity, specifically the number of paths of length $k$ (referred to as $k$-hop paths) connecting two fixed endpoints.
 Multi-hop connectivity plays a central role in applications including wireless networks, device-to-device and/or station-to-device communications in cellular networks, and spread of infection.
...
%   \caption{Unit disk random-connection model.}
 ...
 \end{figure}
   ...
   \noindent
% \noindent
Specifically, we examine a system where nodes are distributed over the interval 
   $[0, kr]$, with $k \geq 2$, according to a binomial point process $\eta$. This process is constrained such that $m_i$ points are located within each sub-interval (or ``cell'') $[(i-1)r, ir]$, $i=1, \ldots, k-1$. Two nodes located at positions $s$ and $t$ are connected by an edge independently with probability $p\in[0,1]$ if and only if $|t-s| \leq r$.
   
\medskip

Within this framework, a $k$-hop path is defined as a sequence of edges connecting two fixed endpoints through exactly $d:=k-1$ intermediate nodes.
We investigate the random variable $\sigma_k(t)$, which counts the number of $k$-hop paths connecting the fixed endpoints located at $0$ and $t \in [(k-1)r, kr]$. An exact expression for the distribution of $\sigma_k(t)$
 was previously obtained via a combinatorial approach in \cite{giles-privault}.
The present work focuses on the asymptotic behaviour of $\sigma_k(t)$ as the number of points in the underlying binomial point process tends to infinity.
\medskip

To this end, we employ the method of cumulants, see, for example, \cite{rudzkis, saulis, doring, thale18, doering}. This technique has recently proven successful in studying moderate deviations in random geometric graphs and weighted random-connection models \cite{schulte-thaele, heerten}, as well as normal approximations for subgraph counts in multidimensional random-connection models \cite{LiuPrivault}. Related results in settings with fixed endpoints can be found in \cite{LiuPrivault24}.

 \medskip
We first derive a combinatorial expression for the joint moments for $k$-hop counts for varying endpoints locations within $[(k-1)d,rk]$, see Proposition~\ref{p1}.
This result is then specialized in Proposition~\ref{djklfs2}
 and Corollary~\ref{djklfs2-2}, where explicit expressions for the variance are obtained.

\medskip

Subsequently, we focus on the homogeneous case where $m_1=m_2=\cdots=m_d=m$ for the asymptotic analysis.
Let $\P_m$ denote the distribution of the one-dimensional unit disk model with $m$ points per cell and connection probability $p_m$. For $n\ge2$, we write $\kappa_n\big(\widebar{\sigma}_k (t)\big)$ for the cumulant of order 
$n$ of the normalized $k$-hop count
$$ 
   \widebar{\sigma}_k (t):= \frac{\sigma_k (t)-\E_m [ \sigma_k (t)]}{\sqrt{\Var_m [ \sigma_k (t) ]}}. 
$$ 
In Section~\ref{sec6}, we derive a general cumulant bound for $\sigma_k(t)$; see Theorem~\ref{jkldd12},
 which is further refined in Theorem~\ref{th7.5-1} in terms of
 the connection probability $p_m$. 
 Combined with the asymptotic variance established in Proposition~\ref{prop7.6}, 
 this leads to the following cumulant bound, stated in Corollary~\ref{cjkfl} 
\begin{equation*}
    \kappa_n\big(\widebar{\sigma}_k (t)\big)\leq \begin{cases}
      \displaystyle
      \frac{C n!^{d+1}}{(m p_m)^{n/2-1}} & \text{when } p_m \gg m^{-1+1/d},
      \medskip
      \\
      \displaystyle
      \frac{C n!^{d+1}}{\big(m^d p_m^{d+1}\big)^{n/2-1}} & \text{when } p_m\ll m^{-1+1/d},
    \end{cases}
\end{equation*}
$n\ge3$, where $C$ is a constant independent of $m \geq 1$. 

Applying the method of cumulants, we establish in Corollary~\ref{thm4.2-1} the convergence rate of the normalized $k$-hop count $\widebar{\sigma}_k (t)$ to the standard normal distribution under the Kolmogorov distance as $m$ tends to infinity.

  \medskip

In addition, we establish in Theorem~\ref{poi-thm} a Poisson approximation for the $k$-hop count using the Stein--Chen method. Taken together, the moment--cumulant analysis reveals a phase transition between Gaussian and Poisson limiting regimes, depending on the scaling of the connection probability $p_m$ relative to the node density.
...
Throughout, we denote by $\Phi$ the cumulative distribution function of the standard normal distribution.

\medskip

For $d\geq 2$ and a fixed endpoint $t\in [(k-1)r,kr]$, we identify the following
 regimes as $m$ tends to infinity. 
\begin{enumerate}[i)] 
\item {\bf Dense regime},
  $p_m \gg m^{-1+1/d}$.
           
  We have $\lim_{m\to\infty}\P_m (\sigma_k (t) \geq 1)=1$, 
  see Corollary~\ref{graphcontain}-$i)$,
  and by Corollary~\ref{thm4.2-1}, 
  the normalized count satisfies the Kolmogorov bound 
\begin{equation*}
      \sup_{x\in\real}\left|
      \P_m \left(
      \widebar{\sigma}_k (t) \leq x\right)-\Phi(x)\right|\leq
	\displaystyle
	\frac{C_d}{(mp_m)^{1/(2+4d)}}
    \end{equation*}
    for $m$ large enough, 
    where $C_d>0$ depends only on
    $d \geq 2$.
\item {\bf Mesoscopic regime},
  $m^{-1+1/d}\gtrsim p_m\gg m^{-d/(d+1)}$.
...
\begin{equation*}
      ...
      \widebar{\sigma}_k (t) \leq x\right)-\Phi(x)\right|\leq
		\displaystyle
	\frac{C_d}{(m^dp_m^{d+1})^{1/(2+4d)}}
    \end{equation*}
    for $m$ large enough, 
    where $C_d>0$ depends only on
    $d \geq 2$.
\item {\bf Thermodynamic regime},
 $p_m \asymp m^{-d/(d+1)}$.

 As $m$ tends to infinity,
 by Corollary~\ref{c2} and \eqref{dtv}
 we have
\begin{align}
	\dtv(\sigma_k (t),\Pn(\lambda_k (m,t)))&
  \asymp \frac{1}{m^{1/(d+1)}},
\nonumber % \label{po-error1}
\end{align}
where $\Pn(\lambda_k (m,t))$ denotes
  the Poisson distribution with parameter
  $\lambda_k (m,t):=\E_m[\sigma_k(t)]$,
  and $\dtv$ is the total variation distance
 defined as 
\begin{equation}
  \label{dtv}
  \dtv( X,Y):=\sup_{A\subseteq\Z}|\P (X\in A)-\P (Y\in A)|
\end{equation}
...
\section{Binomial random-connection model and $k$-hop counts} 
% \label{sec4}
...
\begin{equation}
  \nonumber 
	\sigma_k (t):=\sum_{\beta\in[m_1]\times\cdots\times[m_{d}]}\bone\left(0\leftrightarrow X_{\beta(1)}^{(1)}\right)\left(\prod_{\ell=1}^{k-2}\bone\left(X_{\beta(\ell)}^{(\ell)}\leftrightarrow X_{\beta(\ell+1)}^{(\ell+1)}\right)\right)\bone\left(X_{\beta(d)}^{(d)}\leftrightarrow t\right),\label{def-khop1}
\end{equation}
with $[n]:=\{1,2,\dots,n\}$.
...
Therefore, it can be mapped to a
 ...
 Hence, the count $\sigma_k (t)$ in \eqref{def-khop1} of $k$-hop 
 connecting $0$ to $t\in [(k-1)r,kr]$ 
 can be rewritten as a generalized $U$-statistics of order $d$
\begin{equation}
\label{ustat} 
...
% As above, we let $\lambda_l(s):=\lambda ( s)$, $l=1,\ldots , d$. 
 see Figure~\ref{fig:diagram0-1-2}-$(b)$.  
% \vspace{-0.3cm}
\begin{figure}[H]
\captionsetup[subfigure]{font=footnotesize}
\centering
...
\noindent 
 ...
 for non-crossing partitions. 
       \begin{lemma}
         \label{fkla12}
          \noindent
        $i)$ The cardinality of the set \ ${\rm CNC}([n]\times [d])$
      ...
      $$
      \mathcal{M}(n,d):=\big\{\rho\in{\rm CNC}([n]\times [d]): |\rho|=1+(d-1)n\big\}
      $$
      of maximal connected non-crossing partition of $[n]\times[d]$
      is given by $|\mathcal{M}(1,d)|=1$ for $n=1$, and by               
      \begin{equation}
        \label{coeff-10}
        |\mathcal{M}(n,d)|=d
      \end{equation}
...
\begin{Proof}
\noindent
 $i)$ We clearly have $|{\rm NC} (1,d) | =1$ for all $d\geq 1$. 
...
\begin{definition} 
  Let $j\ge1$, and consider a family
  $$
  f^{(i)}:[0,r]^d \to\real, \quad i=1,\dots,j,
  $$ 
...
\end{equation}
\item for any partition $\rho\in \Pi([j] \times [d] )$, we denote by 
        $$
        \left(
...
\label{p1}
  ...
  the joint moments of $\sigma_k(t_i)$ are given by 
\begin{align} 
\label{djkls12}
...
\left(
 ...
 u_{t_i} \right)_{\! \! \! \rho}
 ( x_1, \ldots , x_{|\rho|} ) 
 \prod_{j=1}^{|\rho|} \mu_{\bar{b}_j} ( \mathrm{d} x_j ), 
\end{align} 
where
\begin{enumerate}[i)]
  \item 
 any partition $\rho=\{b_1,\dots,b_{|\rho |} \}\in {\rm NC} ([n]\times[d])$
 is written as 
$\rho=\bigcup_{i=1}^d\rho_i$ 
 (see \eqref{fjklds143}),
\item
  $\bar{b}_i$ denotes the unique $k\in \{1,\ldots ,d\}$ such that $b_i\in\rho_k$,
  and 
\item we let
%$u_t : [0,r]^d \mapsto \{0,1\}$ is the indicator function of $d$ variables given by 
\begin{equation}
\label{jklf12} 
 u_t (x_1 , \ldots , x_d)
...
\noindent
 ...
 ranges from $d$ to $2d$ and every block has size either one or two,
 see Figure~\ref{f3-1-1} for $d=8$. 
...
\end{equation}
  ...
  and having precisely $q$ blocks, and 
\begin{equation}
...
\begin{Proof}
  ...
  ([m]\times \cdots \times [m])^n$ is disconnected, i.e. 
  ...
  \begin{align}
    ...
    )&= \sum_{\alpha_i\in [m]\times \cdots \times [m]
  \atop
 1 \leq i \leq n
    }
      \kappa(
...
\begin{align*}
  ...
  \atop
      ...
      \kappa(u_{t} (\mathbf{U}_{\alpha_1})
  ,
  \ldots
  ,
  u_{t} (\mathbf{U}_{\alpha_n})
  )
\\ 
   &\le
n^{n-1}
...
% \\ & \leq |m|^{1+(d-1)n} n^{n-1}|{\rm CNC}([n]\times [d])|, 
 \end{align*}
$m\geq 2 ( 1 + (d-1)n)$,
which yields \eqref{fjkldf} from Lemma~\ref{fkla12}-$i)$. % below. 
\end{Proof}
 We note that 
\begin{equation}
  \nonumber 
...
Let $d\geq 2$ and $t\in [(k-1)r,kr]$.
\begin{enumerate}[i)]
\item {\bf Dense regime}, $p_m\gg m^{-1+1/d}$.
 ...
 \begin{equation}
  ...
  \end{equation}
\item {\bf Mesoscopic and sparse regimes},
 $p_m \lesssim m^{-1+1/d}$.
 For $m$ large enough, we have 
    \begin{equation}
      \label{upp-2}
...
\noindent
$ii)$ For all $q=d+1,\ldots, 1+(d-1)n$, we have
\begin{align}
  \label{fjkldfaa1-2}
...
m^{-(q-d)}p_m^{-(q-d)-(n-1)},\ \ \ \ &\mbox{for}~d+n\le q\le 1+(d-1)n,
  ...
  \left(mp_m\right)^{-(q-d)}p_m^{-(n-1)},\ \ \ \ \ &\mbox{for}~d+n\le q\le 1+(d-1)n. 
   \end{cases}
\end{align}
Hence, when $p_m\ll m^{-1}$,
  for all
 $q=d+1,\ldots , 1+(d-1)n$, we have
$$
m^dp_m^{d+1}\gg m^q p_m^{\zeta(n,d,q)}. 
$$
On the other hand,
  when $m^{-1+1/d}\gtrsim p_m\gtrsim m^{-1}$, we have $mp_m^2\ll 1$ since
  $d\ge2$, and $mp_m\gtrsim1$,
 hence for all $d+n\le q \le 1+(d-1)n$, 
 we find that 
\begin{align*}
	(mp_m)^{-(q-d)}p_m^{-(n-1)}&\gtrsim (mp_m)^{-(d-1)(n-1)}p_m^{-(n-1)}\\
  &
  = ( m^{ -1 + 1/d }p_m^{-1} )^{(n-1)d}
  \\
   & \gtrsim 1, 
\end{align*}
uniformly in
  $q = d+n, \ldots , \le 1+(d-1)n$. 
  We conclude from \eqref{cum-asy1} that when $p_m\ll m^{-1+1/d}$,
 for $q=d+1,\dots,1+(d-1)n$ we have 
$$
m^dp_m^{d+1}\gtrsim m^qp_m^{\zeta(n,d,q)},
$$
uniformly in
  $q = d+n, \ldots , \le 1+(d-1)n$. 
 Hence, given that $\zeta (n,d,d) = d+1$,
 when $p_m\ll m^{-1+1/d}$ 
 we obtain from \eqref{fjkldf} that for $m$ large enough,
\begin{align*}
 \vert\kappa_n(\sigma_k (t))\vert &\leq n^{n-1}
  \sum_{q=d}^{1+(d-1)n} |\mathcal{C}(n,d,q)|m^q p_m^{\zeta (n,d,q)}\\
  &\lesssim n^{n-1} m^dp_m^{d+1} 
  \sum_{q=d}^{1+(d-1)n} |\mathcal{C}(n,d,q)|\\
...
% with $v(G)=k$ vertices,
  Let $d\geq 2$ and $t\in [(k-1)r,kr]$.
  \begin{enumerate}[i)]
\item If $p_m\geq m^{-1+1/d}$, $m\geq 1$, then we have 
  \begin{equation}
    \nonumber % \label{upp-1}
    |\kappa_n(\sigma_k (t))|\leq
    n^{n-1}n!^d m^{1+(d-1)n}p_m^{nd+1},
     \quad m \geq 1. 
  \end{equation}
\item If $p_m\leq m^{-1+1/d}$, $m\geq 1$, then we have 
    \begin{equation}
    \nonumber %   \label{upp-2}
      |\kappa_n(\sigma_k (t))|\leq
      n^{n-1}n!^d 
...
\noindent
 $i)$ If $p_m \gg m^{-1+1/d}$, the dominant term
 in $\E_m  [ ( \sigma_k(t) )^2 ]$ 
...
$$
\smallskip
$ii)$ If $p_m \lesssim m^{-1+1/d}$, the term 
$$ 
 ( \E_m [ \sigma_k (t) ] )^2 = 
p_m^{2d+2} m^{2d} \frac{((kr-t) / r)^{2d}}{(d!)^2}, 
$$ 
 ...
 satisfies 
  \begin{eqnarray*}
    \kappa_n\left(\widebar{\sigma}_k (t)\right)\leq \begin{cases}
      \displaystyle
      \frac{C n!^{d+1}}{(m p_m)^{n/2-1}},~~~~~~~&\mathrm{when}~~~p_m \gg m^{-1+1/d},
      \medskip
      \\
      \displaystyle
                                                      \frac{C n!^{d+1}}{\big(m^d p_m^{d+1}\big)^{n/2-1}},~~~~&\mathrm{when}~~~p_m\lesssim m^{-1+1/d},
    \end{cases}
\end{eqnarray*}
 $n\ge3$, where $C>0$ is a constant independent of $m \geq 1$.
\end{corollary}
\begin{Proof}
% From the inequality $\sqrt{2\pi j}({j}/{e})^j<j!$, we have
% \begin{equation} j^{j-1}<\frac{(j-1)!}{\sqrt{2\pi j}}e^j. \end{equation}
  $i)$ When $p_m \gg m^{-1+1/d}$, from the inequalities  
  \eqref{upp-1} and \eqref{low-1} we have 
  \begin{align*}
    \nonumber
...
%\begin{remark}
%  To ensure normal approximation, we only need to let some $m_\ell$ tend to infinity, i.e.
  % $M$ tends to infinity,  $\max_{1\leq \ell \leq d} m_\ell$ should tend to infinity,   
%which does not require 
 % $m_1=\cdots=m_{k-1}=m$ as $m$ tends to infinity. 
%\end{remark}
For any $t \in [(k-1)r,kr)$, from Corollary~\ref{cjkfl} the skewness
 of the normalized $k$-hop count
$
\widebar{\sigma}_k (t)
$
 satisfies  
  \begin{eqnarray*}
    \kappa_3\left(\widebar{\sigma}_k (t)\right)\leq \begin{cases}
      \displaystyle
      \frac{C}{(m p_m)^{1/2}}~~~~~~~&\mathrm{when}~~~p_m \gg m^{-1+1/d},
      \medskip
      \\
      \displaystyle
      \frac{C}{(m^d p_m^{d+1})^{1/2}}~~~~&\mathrm{when}~~~p_m\lesssim m^{-1+1/d},
    \end{cases}
\end{eqnarray*}
  for $m$ large enough, where $C>0$ is a constant independent of $m \geq 1$.
   More generally,
  from Corollary~\ref{cjkfl} and Theorem~1 in \cite{Janson1988}, 
  we find that 
 $\widebar{\sigma}_k (t)$
 converges in distribution
 to the standard normal distribution ${\cal N}(0,1)$ as $m$
 tends to infinity, 
 provided that $p_m \gg m^{-d/(d+1)}$ as $m$ tends to
 infinity.

 \medskip
 
 More precisely, from Corollary~\ref{cjkfl}, we check that if
 $p_m \gg m^{-d/(1+d)}$, the cumulants of the normalized
 $U$-statistics 
 ...
 \Delta_m :=
\begin{cases}
	C(mp_m)^{1/2},~~~~~~~&\mathrm{when}~~~p_m\gg m^{-1+1/d},
	\medskip
	\\
	\displaystyle
	C(m^dp_m^{d+1})^{1/2},~~~~~~~&\mathrm{when}~~~m^{-1+1/d}\gtrsim p_m\gg m^{-d/(d+1)},
\end{cases}
$$ 
...
\noindent
Let $d\geq 2$, $t\in [(k-1)r,kr]$. Then, for $m$ large enough, we have 
    \begin{equation*}
      \sup_{x\in\real}\left|
      \P_m \left(
      \widebar{\sigma}_k (t) \leq x\right)-\Phi(x)\right|\leq
\begin{cases}
	\displaystyle
	\frac{C_d}{(mp_m)^{1/(2+4d)}},~~~&\mathrm{when}~~p_m\gg m^{-1+1/d},
	\medskip
	\\
		\displaystyle
	\frac{C_d}{(m^dp_m^{d+1})^{1/(2+4d)}},~~~&\mathrm{when}~~m^{-1+1/d}\gtrsim p_m\gg m^{-d/(d+1)},
\end{cases}
    \end{equation*}
where $C_d>0$ depends only on
    $d \geq 2$. 
...
\begin{enumerate}
	\item[i)]
if
$p_m \sim m^{-\alpha}$ with $\alpha \in
\displaystyle \left[0,1 - \frac{1}{d}\right)$, we have 
    \begin{equation}
\nonumber 
      \sup_{x\in\real}\left|
      \P_m \left(
...
\item[ii)]
  ...
  with $\alpha \in
  \displaystyle
  \left[1 - \frac{1}{d},\frac{d}{d+1}\right)$, we have 
    \begin{equation}
\nonumber 
      \sup_{x\in\real}\left|
      \P_m \left(
      \widebar{\sigma}_k (t) \leq x\right)-\Phi(x)\right|\leq \frac{C_d}{m^{[d-\alpha(d+1)]/(2+4d)}},
    \end{equation}
    where $C_d>0$ depends only on
    $d \geq 2$;
\end{enumerate}
% \noindent 
 ...
 In the case $d=1$, the central limit Berry-Esseen rate holds
 since $\sigma_2(t)$ has a binomial distribution
 with parameter in \eqref{d=1}.
...
\label{graphcontain}
    ...
    We have 
    \begin{enumerate}[i)]
	    \item {\bf Dense and Mesoscopic regimes}, $p_m \gg m^{-d/(d+1)}$.
 We have $$\lim_{m\to\infty}\P_m (\sigma_k (t) \geq 1)=1.$$ 
\item {\bf Sparse regime}, $p_m\ll m^{-d/(d+1)}$. 
 We have $\lim_{m\to\infty}\P_m (\sigma_k (t) =0)=1$. 
    \end{enumerate}
  \end{corollary}
  \begin{Proof}
 When $p_m\gg m^{-1+1/d}$,
...
\lim_{m\to \infty}
   \frac{m^{2d-1}p_m^{2d+1}}{m^{2d}p_m^{2d+2}}
   = 0, % \to0,
 \end{equation}
 and
 \begin{eqnarray}
   ...
   = 1.
 \end{eqnarray}
Moreover, when $m^{-1+1/d}\gtrsim p_m\gg m^{-d/(d+1)}$, by \eqref{upp-2}, 
	 $$
	 |\kappa_2(\sigma_k(t))|\lesssim 2^{d+1}m^dp_m^{d+1},
	 $$
	 and 
\begin{equation}
      \lim_{m\to \infty}
   \frac{\kappa_2(\sigma_k (t) )}{\kappa_1(\sigma_k (t) )^2} \lesssim 
\lim_{m\to \infty}
   \frac{m^{d}p_m^{d+1}}{m^{2d}p_m^{2d+2}}
   = 0.
\end{equation}
 On the other hand, if $p_m\ll m^{-d/(d+1)}$, from
...
To study the Poisson approximation of the $k$-hop count $W$, we employ the
 well-established
 Stein-Chen method originally developed in 
...
construct
 a family 
 $\{\{X_j^{(i)} : j \in \ci\} : i \in \ci\}$
 of Bernoulli random variables satisfying 
 \eqref{sizebiasedcond} and 
$X_j^{(i)} \le X_j$ (resp. $X_j^{(i)} \ge X_j$) for all $j \ne i$.
\end{definition}
 When the  $X_i$'s are negatively related, we have 
\begin{equation}
...
% = \mu^{-1}(\mu - \sigma^2),
   = 1 - \frac{\sigma^2}{\mu},
\end{equation}
where $\mu := \E_m [W]$ and $\sigma^2 := \var_m [ W ]$.
...
\begin{align}\label{monotonecoupling2}
   ...
   &\le \E_m  \bigg[
     \sum_{j \ne B} (X_j^{(B)} - X_j) + X_B\bigg] \nonumber\\
   &= \E_m  [ W^s - W - 1 ] + \frac{2}{\mu} \sum_{i \in \ci} \bar{p}_i^2 \nonumber\\
   &= - 1 + \frac{\sigma^2}{\mu} + \frac{2}{\mu} \sum_{i \in \ci} \bar{p}_i^2.
\end{align}
% \subsubsection*{Stein-Chen method}
 In what follows, we consider the family
...
\end{equation*}
\end{enumerate}
It is easy to verify that $\law(I_{\beta}^{(\alpha)})=\law\left(I_\beta\big|I_\alpha=1\right)$. In addition, it is straightforward by construction
...
\begin{theorem}\label{poi-thm}
  ...
  \eqref{ibeta} and $\lambda_k (m,t):=\E_m  [ \sigma_k (t) ]>0$.
  Then, we have
\begin{align}
  \dtv(\sigma_k (t),\Pn(\lambda_k (m,t)))&\le
  \tilde{p}_m
  + 
  \sum_{i=1}^{d-1}\nu(i)p_m^{d+1-i},
  \label{po-error1}
...
\begin{align}
\left|\E[\lambda f_A(W+1)-Wf_A(W)]\right|&= \lambda \left| \E[f_A(W+1)-f_A(W^s)]\right|\nonumber\\
				    &\le\lambda \|\Delta f_A\| \ \! \E [ |W+1-W^s| ] \nonumber\\
				    &\le\lambda^{-1}(\sigma^2 - \lambda) + 2\lambda^{-1} \sum_{\alpha \in \ci} (\P_m (I_\alpha=1))^2\nonumber\\
&=
2\tilde{p}_m + \frac{\sigma^2}{\lambda} - 1,
\label{err01}
\end{align}
where $\sigma:=\Var [ W]$. 
Moreover, we have 
...
\begin{align}
  ...
  -\lambda\tilde{p}_m
  +
  \sum_{\alpha\in\ci}\sum_{i=1}^{d-1}\sum_{\substack{\beta\in\ci\\|\underline{\beta}\cap\underline{\alpha}|=i}}\cov(I_\alpha,I_\beta)
  \\
  \nonumber
  &\le
  -\lambda\tilde{p}_m
  + \sum_{\alpha\in\ci}\sum_{i=1}^{d-1}\sum_{\substack{\beta\in\ci\\|\underline{\beta}\cap\underline{\alpha}|=i}}\left(\tilde{p}_mp_m^{d+1-i}-\tilde{p}_m^2\right)
  \\
  \nonumber
  &\le
  -\lambda\tilde{p}_m
  + \sum_{\alpha\in\ci}\sum_{i=1}^{d-1}\sum_{\substack{\beta\in\ci\\|\underline{\beta}\cap\underline{\alpha}|=i}}\tilde{p}_mp_m^{d+1-i}
  \\
  \nonumber
  &=
  -\lambda\tilde{p}_m
  + \sum_{\alpha\in\ci}\sum_{i=1}^{d-1}\nu(i)\tilde{p}_mp_m^{d+1-i}
  \\
  \nonumber
  &=
  -\lambda\tilde{p}_m
  +
  \lambda\sum_{i=1}^{d-1}\nu(i)p_m^{d+1-i}.\label{err05}
\end{align}
Combining 
...
\end{Proof}
 ...
 Poisson approximation by Stein-Chen method does not require $\E_m  [\sigma_k (t)]$
  to be a constant. In fact, $\E_m  [\sigma_k (t)]$ can be large.
  \begin{corollary}
    \label{c2} 
    (Poisson convergence, thermodynamic regime).
 
    \noindent
    Let $d\geq 2$ and $t\in [(k-1)r,kr]$,
     assume that $m_1=m_2=\cdots=m_d=m$,
     and that $p_m \asymp m^{-d/(d+1)}$. 
% $$ \lim_{m\to \infty} m^dp_m^{d+1} = c\in(0,\infty). $$ 
...
\end{corollary}
\begin{Proof}
  By \eqref{count-parti} we have $\nu(i)\asymp m^{d-i}$,
   hence the error bound in \eqref{po-error1} is of order 
   $$\sum_{i=1}^{d-1}m^{d-i}p_m^{d+1-i}+p_m^{d+1}\asymp \sum_{i=1}^{d-1}m^{d-i}
   \big( m^{- d / ( d+1) } \big)^{d+1-i}\asymp m^{-1/(d+1)}.$$
\end{Proof}
 In the case $d=1$, 
 Poisson convergence also holds from
 \eqref{d=1} for $\sigma_2(t)$ when
 $\lim_{m\to \infty} mp_m^2 = c\in(0,\infty)$.
...
$\mathbf{X}_b$ is defined as 
 ...
 is defined as 
 \begin{equation*}
\kappa(\mathbf{X}_b)=(-i)^k\frac{\partial^k}{\partial t_1\cdots\partial t_k}\ln \varphi_{\mathbf{X}_b}(t_1,\dots,t_k)_{\big| t_1=\cdots=t_k=0}. 
\end{equation*}
...
\item (Kolmogorov bound,
 \cite[Corollary~2.1]{saulis} and \cite[Theorem~2.4]{doering})
  One has
\begin{equation*}
\sup_{x\in\real }|\P_m (X_n\leq x)-\Phi(x)|\leq \frac{C_\gamma}{(\Delta_n)^{1/(1+2\gamma)}},
\end{equation*}
for some $C_\gamma >0$ depending only on $\gamma$.
...
\bibitem[CCCB25]{bhattacharya25}
S.~Chatterjee, A.~Chatterjee, A.~Chakraborty, and B.~Bhattacharya.
\newblock Asymptotic normality of subgraph counts in sparse inhomogeneous
  random graphs, 2025.
\newblock Preprint arXiv:2512.12937.
...
\bibitem[FLL{\etalchar{+}}05]{foh2005}
C.H. Foh, G.~Liu, B.S. Lee, B.-C. Seet, K.-J. Wong, and C.P. Fu.
\newblock Network connectivity of one-dimensional {MANETs} with random waypoint
  movement.
\newblock {\em {IEEE} Communications Letters}, 9(1):31--33, 2005.
...
\bibitem[HM07]{han2007}
G.~Han and A.~Makowski.
\newblock A very strong zero-one law for connectivity in one-dimensional
  geometric random graphs.
\newblock {\em {IEEE} Communications Letters}, 11(1):55--57, 2007.
...
\bibitem[LMD67]{liebreview}
E.H. Lieb, D.C. Mattis, and F.J. Dyson.
\newblock Mathematical physics in one dimension: Exactly soluble models of
  interacting particles.
\newblock {\em Physics Today}, 20(9):81--82, 1967.
...
\bibitem[NZZ{\etalchar{+}}11]{ng2011}
S.C. Ng, W.~Zhang, Y.~Zhang, Y.~Yang, and G.~Mao.
\newblock Analysis of access and connectivity probabilities in vehicular relay
  networks.
\newblock {\em IEEE J. Sel. Areas Commun.}, 29(1):140--150, 2011.
...
\bibitem[Sha09]{shang2009}
Y.~Shang.
\newblock Connectivity in a random interval graph with access points.
\newblock {\em Information Processing Letters}, 109(9):446--449, 2009.
...
\bibitem[WDG20]{wilsher}
M.~Wilsher, C.P. Dettmann, and A.~Ganesh.
\newblock Connectivity in one-dimensional soft random geometric graphs.
\newblock {\em Phys. Rev. E}, 102:062312, 2020.

\bibitem[ZCY{\etalchar{+}}12]{zhang2012}
W.~Zhang, Y.~Chen, Y.~Yang, X.~Wang, Y.~Zhang, X.~Hong, and G.~Mao.
\newblock Multi-hop connectivity probability in infrastructure-based vehicular
  networks.
\newblock {\em IEEE J. Sel. Areas Commun.}, 30(4):740--747, 2012.
...
\bibitem[ZMA14]{zzhang2014}
Z.~Zhang, G.~Mao, and B.D.O. Anderson.
\newblock Stochastic characterization of information propagation process in
...
d_{K/W} \big( \widebar{\sigma}_k (t) , {\cal N} \big)
\leq \frac{C_{k,r}}{\sqrt{\lambda}}
\end{equation} 
 for some constant $C_{k,r}>0$, as $\lambda$ tends to infinity. 
\end{prop}
\begin{Proof}
 The kurtosis of $\sigma_k (t )$ satisfies 
\begin{eqnarray*} 
 \frac{\E_\lambda [ ( \sigma_k(t) -  \E_\lambda [ \sigma_k(t) ] )^4 ]}{( \Var_\lambda [ \sigma_k (t)] )^2} - 3
 & = & 
 \E_\lambda \big[ \big( \widebar{\sigma}_k (t) \big)^4 \big] - 3 
 \\
 & = & 
...
\leq \frac{C_{k,r}}{\lambda^{1/(2 + 4(k-3))}}
\end{equation} 
