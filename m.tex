...
% \noindent
 ...
 cell $[(i-1)r, ir]$, $i=1,\ldots , k-1$, $k\geq 2$. 
 ...
 a path connecting two fixed points $s,t\in [0,kr]$
...
expressed by a combinatorial approach in \cite{giles-privault}. 
 We will consider the asymptotic behavior of the count statistics
 $\sigma_k (t)$ as the number of binomial points tends to infinity.
...
% \cite{jansen},
 ...
 see \cite{LiuPrivault}, and \cite{LiuPrivault24}
 in the presence of endpoints.  
...
\medskip
 ...
 Taking $m_1=m_2=\cdots=m_d=m \geq 1$ and 
 denoting by $\kappa_n\left(\widebar{\sigma}_k (t)\right)$
 the cumulant of order $n\geq 2$ of the normalized $k$-hop count
 $$ 
   \widebar{\sigma}_k (t):= \frac{\sigma_k (t)-\E_m [ \sigma_k (t)]}{\sqrt{\Var_m [ \sigma_k (t) ]}}, 
  $$ 
% when every cell contains $m$ nodes
 with connection probability $p_m$,
 in Corollary~\ref{cjkfl} 
  we obtain the bound 
  \begin{eqnarray*}
...
\end{eqnarray*}
 ...
 in Corollary~\ref{thm4.2-1} we obtain 
 the bound \eqref{fjkf}
 for the convergence of the normalized $k$-hop count 
$\widebar{\sigma}_k (t)$ 
to the standard normal distribution % ${\cal N}$ 
as $m$ tends to infinity,
using the Kolmogorov distance. % s $d_W(X,Y)$ and
% $$ d_W (X,Y): =\sup_{h\in\mathrm{Lip}(1)} |\mathrm{E}[h(X)]-\mathrm{E}[h(Y)]|, $$ where $\mathrm{Lip}(1)$ denotes the class of real-valued Lipschitz functions with Lipschitz constant less than or equal to $1$, and
% $$d_K(X,Y) : = \sup_{x\in \real} | \P_m ( X \leq x ) - \P_m ( Y \leq x ) | $$  between the distributions of two random variables $X$, $Y$. 
In Theorem~\ref{poi-thm} we also provide a Poisson approximation of the $k$-hop count via the Stein-Chen method.

\medskip
 
 We will use the following standard notation for the asymptotic behavior of the relative order of magnitude of two functions $f(x)$ and $g(x)>0$ as $x$ tends to infinity.
...
\end{itemize}
 ...
 We also denote by $\Phi$ the cumulative distribution function of the
...
\item 
  ...
  If $p_m \gg m^{-1+1/d}$,
    ...
    see Corollary~\ref{graphcontain}-$a)$,
    and the Kolmogorov bound 
    \begin{equation}
   \nonumber % \label{fjkf}
...
% see Figure~\ref{fklds} for an example with $k=5$, $x=0$, $t=4.5$, and $r=1$.
...
\end{align} 
 ...
 $\rho\in {\rm NC} ([n]\times[d])$
 is written as 
 $\rho=\{b_1,\dots,b_{|\rho |} \}$ and 
% $$c(\rho ) : = |b_1|+|b_d|+\sum_{i=1}^{d-1} \min ( |b_i|,|b_{i+1}|), $$ 
 $u_t : [0,r]^d \mapsto \{0,1\}$ is the indicator function
...
\end{equation} 
 and $\bar{b}_i$
 denotes the index $k\in \{1,\ldots ,d\}$
 of the partition $\rho_k$ containing $b_i$,
 see \eqref{fjklds143}. 
% denotes the index $j\in \{1,\ldots , d \}$ of the unique block $\eta_j = ((i,j))_{i=1,\ldots , n}$ containing $\rho_i$, $i=1,\ldots , |\rho|$. 
